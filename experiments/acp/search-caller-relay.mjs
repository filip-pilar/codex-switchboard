...
try{let result;if(m.method==='initialize')result={protocolVersion:m.params.protocolVersion,capabilities:{tools:{listChanged:true}},serverInfo:{name:'codex-caller-relay',version:'0.1.0'}};
else if(m.method==='tools/list')result=await relay('/tools',{});
else if(m.method==='tools/call')result=await relay('/call',m.params);
else throw Error('unsupported method');const refresh=result.refreshTools;delete result.refreshTools;process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:m.id,result})+'\n');if(refresh)process.stdout.write(JSON.stringify({jsonrpc:'2.0',method:'notifications/tools/list_changed'})+'\n');}
