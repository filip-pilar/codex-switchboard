...
async function handle(m){
 ...
 const tc={...(toolCalls.get(p.toolCall?.toolCallId)||{}),...(p.toolCall||{})};row.inferenceName=tc._meta?.['cognition.ai/inferenceToolName'];row.kind=tc.kind;row.toolKeys=Object.keys(tc);row.inputKeys=Object.keys(tc.rawInput||{});row.fixtureInput=Object.entries(tc.rawInput||{}).filter(([k,v])=>typeof v==='string'&&v===target).map(([k])=>k);row.locationMatches=(tc.locations||[]).map(l=>l.path===target);row.optionKinds=(p.options||[]).map(o=>o.kind);
 const locations=tc.locations||[];const input=tc.rawInput||{}; const allowed=(webAllowed&&tc._meta?.['cognition.ai/inferenceToolName']==='web_search')||toolSpecs.some(t=>t.type==='function'&&tc._meta?.['cognition.ai/inferenceToolName']==='mcp__codex__'+t.name)||(tc.kind==='edit'&&locations.length>0&&locations.every(l=>l.path===target))||(input.file_path===target&&['BEFORE_EDIT','BEFORE_EDIT\n'].includes(input.old_string)&&['AFTER_EDIT','AFTER_EDIT\n'].includes(input.new_string));
 ...
 if(m.method==='session/update'){const u=m.params?.update||{};if(u.toolCallId){const merged={...(toolCalls.get(u.toolCallId)||{}),...u};toolCalls.set(u.toolCallId,merged);if(merged._meta?.['cognition.ai/inferenceToolName']==='web_search'){if(typeof merged.rawInput?.query==='string')webQuery=merged.rawInput.query;evidence.webUpdates??=[];evidence.webUpdates.push({status:u.status,keys:Object.keys(u),outputKeys:typeof u.rawOutput==='object'&&u.rawOutput?Object.keys(u.rawOutput):[],metaKeys:Object.keys(u._meta||{}),metaShape:Object.fromEntries(Object.entries(u._meta||{}).map(([k,v])=>[k,v&&typeof v==='object'?Object.keys(v):typeof v])),contentKinds:(u.content||[]).map(c=>c.type)});if(u.status==='completed'){evidence.webCompleted=true;const output=JSON.stringify(u.content||[]);evidence.webSourceMatches=output.includes('modelcontextprotocol.io');evidence.webOutcome={categories:['result','found','error','blocked','denied','failed','success'].filter(w=>output.toLowerCase().includes(w)),counts:[...output.matchAll(/(\d+)\s+(?:results|pages|sources)/gi)].map(m=>Number(m[1])),contentShapes:(u.content||[]).map(c=>({keys:Object.keys(c),innerKeys:Object.keys(c.content||{})}))};}}}const key=u.sessionUpdate||'unknown';evidence.updates[key]=(evidence.updates[key]||0)+1;return;}
...
let webAllowed=false,webQuery;let codex,activeResponse,started=false,toolSpecs=[],finalText='',finished=false;
...
function emit(res,item){
 ...
 const items=item.type==='message'&&evidence.webCompleted&&webQuery?[{id:'ws_'+randomUUID(),type:'web_search_call',status:'completed',action:{type:'search',query:webQuery}},item]:[item];evidence.webItemEmitted=items.some(i=>i.type==='web_search_call');event('response.created',{response});items.forEach((value,index)=>{event('response.output_item.added',{output_index:index,item:{...value,status:'in_progress'}});event('response.output_item.done',{output_index:index,item:value});});event('response.completed',{response:{...response,status:'completed',output:items,usage:{input_tokens:0,output_tokens:0,total_tokens:0}}});res.end();
}
function flush(){
...
const server=http.createServer(async(req,res)=>{
 ...
 evidence.requestMetadata??=[];evidence.requestMetadata.push({path:req.url,model:body.model,keys:Object.keys(body),toolCount:body.tools?.length,toolTypes:(body.tools||[]).map(t=>({type:t.type,name:t.name,keys:Object.keys(t)}))});toolSpecs=flatten(body.tools||[]);webAllowed=(body.tools||[]).some(t=>t.type==='web_search'&&t.external_web_access!==false);evidence.webDeclared=webAllowed;if(!webAllowed){res.writeHead(400,{'content-type':'application/json'}).end(JSON.stringify({error:{code:'live_web_disabled'}}));return;}evidence.callerTools=[...new Set(toolSpecs.map(t=>t.name))];
...
try{
 ...
 codex=spawn(findCodex(),['exec','--ephemeral','--skip-git-repo-check','--sandbox','workspace-write','--color','never','--json','-c',process.env.ACP_WEB_CACHED==='1'?'web_search="cached"':'web_search="live"','-m','acp-model-test','-c','model_reasoning_effort='+JSON.stringify(effort),...(visionMode&&!noImage?['-i',imagePath,...(process.env.ACP_VISION_PAIR==='1'?['-i',imagePath2]:[])]:[]),'--','Use your native web_search tool to search the public web for the official Model Context Protocol tools list-changed notification documentation. Report the official documentation URL and the exact notification method. Do not use MCP, shell, or file tools. You must perform an actual web search.'],{cwd:work,env,stdio:['ignore','pipe','pipe']});
 let buf='';codex.stderr.resume();codex.stdout.on('data',c=>{buf+=c;let at;while((at=buf.indexOf('\n'))>=0){const line=buf.slice(0,at);buf=buf.slice(at+1);try{const e=JSON.parse(line);if(e.item?.type){evidence.codexItemTypes??=[];if(!evidence.codexItemTypes.includes(e.item.type))evidence.codexItemTypes.push(e.item.type);}if(e.item?.type==='file_change'&&e.item.status==='completed')evidence.codexPatchCompleted=true;if(e.item?.type==='command_execution'){evidence.codexCommands??=[];evidence.codexCommands.push({exitCode:e.item.exit_code,status:e.item.status,after:e.item.aggregated_output?.includes('AFTER_EDIT')});}if(e.item?.type==='agent_message')evidence.finalMarker=e.item.text?.includes('CHECK_EDIT_OK');if(e.type==='turn.failed')evidence.codexTurnFailed=true;}catch{}}});
 const [code,signal]=await once(codex,'exit');evidence.codexExitCode=code;evidence.codexSignal=signal;
}finally{
 if(!broker.close(sessionHandle))broker.cancel(sessionHandle);child.kill('SIGTERM');await closed;clearTimeout(timeout);callerRelay.closeAllConnections();await new Promise(r=>callerRelay.close(r));server.closeAllConnections();await new Promise(r=>server.close(r));evidence.officialURL=finalText.includes('modelcontextprotocol.io/');evidence.notificationCorrect=finalText.includes('notifications/tools/list_changed');evidence.fileVerified=fs.readFileSync(target,'utf8')==='AFTER_EDIT\n';fs.rmSync(root,{recursive:true,force:true});evidence.scratchRemoved=!fs.existsSync(root);if(visionMode){const cards=JSON.parse(fs.readFileSync('.build/compat-audit/vision-diagnostic/expected.json'));const expected=cards[card].expected+(process.env.ACP_VISION_PAIR==='1'?cards[1].expected:'');evidence.visionCorrect=finalText.toUpperCase().replace(/[^A-Z0-9]/g,'')===(noImage?'NOIMAGE':expected);evidence.noImage=noImage;evidence.card=card;}evidence.exactResult=finalText.trim()===verificationValue;evidence.containsResult=finalText.includes(verificationValue);evidence.sessionsRemaining=broker.size;evidence.finishedAt=Date.now();writeJSON('.build/compat-audit/'+(process.env.ACP_AUDIT_LABEL||'swe2-session-relay')+'.json',evidence);console.log(JSON.stringify(evidence));
}
