...
import {callerToolName,findCallerTool,isCallerTool} from './tool-names.mjs';
import {prepareDevinSearch,restoreDevinSearchEvent} from '../../gateway/codex/deferred-search.mjs';
...
const callerRelay=http.createServer(async(req,res)=>{try{
 ...
 let result;if(req.url==='/tools'){result={tools:toolSpecs.filter(t=>t.type==='function').map(t=>({name:callerToolName(t),description:t.description||t.name,inputSchema:t.parameters}))};evidence.relayToolsListed=(evidence.relayToolsListed||0)+1;}
 else if(req.url==='/call'){if(!isCallerTool(toolSpecs,body.name))throw Error('undeclared tool');const output=await external('caller',body);result={content:[{type:'text',text:(typeof output==='string'?output:JSON.stringify(output))+(body.name==='switchboard_tool_search'?'\nThese are caller-side tool names. Execute loaded tools through the configured codex MCP server only. Refresh its tool list. Available relay invocation names: '+toolSpecs.filter(t=>t.type==='function'&&t.namespace).map(t=>'mcp__codex__'+callerToolName(t)).join(', '):'')}],refreshTools:body.name==='switchboard_tool_search'};evidence.relayResults=(evidence.relayResults||0)+1;}
...
const add=spawnSync(findCLI('devin'),['mcp','add','codex','--scope','user','--env','RELAY_URL=http://127.0.0.1:'+callerRelay.address().port,'--env','RELAY_CAPABILITY='+capability,'--',process.execPath,path.resolve('experiments/acp/search-caller-relay.mjs')],{cwd:work,env:{...process.env,XDG_CONFIG_HOME:path.join(root,'config'),XDG_DATA_HOME:data,XDG_CACHE_HOME:path.join(root,'cache')},stdio:'pipe'});evidence.mcpAddExit=add.status;
...
async function handle(m){
 ...
 const locations=tc.locations||[];const input=tc.rawInput||{}; const allowed=(tc._meta?.['cognition.ai/inferenceToolName']?.startsWith('mcp__codex__')&&isCallerTool(toolSpecs,tc._meta['cognition.ai/inferenceToolName'].slice('mcp__codex__'.length)))||(tc.kind==='edit'&&locations.length>0&&locations.every(l=>l.path===target))||(input.file_path===target&&['BEFORE_EDIT','BEFORE_EDIT\n'].includes(input.old_string)&&['AFTER_EDIT','AFTER_EDIT\n'].includes(input.new_string));
 ...
 if(m.method==='session/update'){const u=m.params?.update||{};if(u.toolCallId){const merged={...(toolCalls.get(u.toolCallId)||{}),...u};toolCalls.set(u.toolCallId,merged);if(u.sessionUpdate==='tool_call'){evidence.nativeTools??=[];evidence.nativeTools.push({name:merged._meta?.['cognition.ai/inferenceToolName'],inputKeys:Object.keys(u.rawInput||{})});}}const key=u.sessionUpdate||'unknown';evidence.updates[key]=(evidence.updates[key]||0)+1;return;}
...
function emit(res,item){
 ...
 const event=(type,data)=>res.write(`event: ${type}\ndata: ${JSON.stringify(restoreDevinSearchEvent({type,sequence_number:seq++,...data}))}\n\n`);
...
function flush(){
 ...
 const next=broker.take(sessionHandle);if(next){if(process.env.ACP_AUDIT_DISCONNECT==='1'){evidence.disconnectInjected=true;activeResponse.destroy();return;}const task=next.payload;const spec=findCallerTool(toolSpecs,task.kind==='caller'?task.params.name:task.kind==='read'?'exec_command':'apply_patch');
...
const server=http.createServer(async(req,res)=>{
 ...
 const chunks=[];for await(const c of req)chunks.push(c);const rawBody=JSON.parse(decodeBody(Buffer.concat(chunks),req.headers['content-encoding']));evidence.searchAdvertised??=(rawBody.tools||[]).some(t=>t.type==='tool_search');const body=prepareDevinSearch(rawBody).body;
...
try{
 ...
 fs.appendFileSync(path.join(home,'config.toml'),`[mcp_servers.search_lab]\ncommand = ${JSON.stringify(process.execPath)}\nargs = [${JSON.stringify(path.resolve('experiments/acp/search-fixture.mjs'))}]\n[mcp_servers.search_lab.env]\nLAB_VALUE = ${JSON.stringify(verificationValue)}\nLAB_LOG = ${JSON.stringify(path.join(root,'search-calls.jsonl'))}\n`);
 ...
 codex=spawn(findCodex(),['exec','--ephemeral','--skip-git-repo-check','--sandbox','workspace-write','--color','never','--json','-m','acp-model-test','-c','model_reasoning_effort='+JSON.stringify(effort),...(visionMode&&!noImage?['-i',imagePath,...(process.env.ACP_VISION_PAIR==='1'?['-i',imagePath2]:[])]:[]),'--','Discover the codex MCP tools. Use switchboard_tool_search to discover the laboratory beacon calibration tool, then discover the updated codex MCP tools and call the loaded beacon tool. Reply with its exact returned value. Do not use file or shell tools or create subagents.'],{cwd:work,env,stdio:['ignore','pipe','pipe']});
