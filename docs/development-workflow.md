...
| Native account/RPC behavior | `node bin/test-native.mjs` | Swift toolchain; wrapper sets fixture paths and path-specific caches. Discovers the installed runtime using the gateway's lookup (ChatGPT/Codex app or PATH; `CODEX_CLI_PATH` override), or accepts `SWITCHER_TEST_INSTALLED_CLI`. If none is found, only that runtime test is skipped. Uses a private scratch home. |
...
| Helper packaging | `node bin/check-packaging.mjs` | Defaults to `.build/macos/switchboard-helper`; optional first argument selects another helper. Needs port 9477 free and Node 24+ as recommended in the README (`zstdCompressSync` is required; the package's Node >=20 floor alone is insufficient). Uses missing-provider fixtures and scratch restore state. |
