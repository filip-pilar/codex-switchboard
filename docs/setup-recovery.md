...
In Connections, **Options → Use Existing Login** (or **Refresh** when connected) checks authenticated discovery without an inference request. **Sign In… / Options → Sign In Again…** opens the official CLI browser flow. Devin and Grok each use the current official CLI identity. Reconnect pauses that provider, cancels its active requests, and reloads only that worker. Missing CLIs open their official installation instructions; they are not redistributed or automatically upgraded.
...
1. Select compatible models with the model switches. New discoveries stay disabled. Provider-advertised support is separate from prior/live verification.
...
4. Choose **Enable in Codex…**. While active work is running, choose **Apply, Restart Later**. This writes the combined catalog, starts the owned loopback service, and sets native-provider `openai_base_url`/`model_catalog_json`. No installed Codex/ChatGPT bundle is patched.
...
Copy Diagnostics contains versions/readiness, stable error categories and last-route metadata. It excludes tokens, auth headers, account emails, raw provider errors and request/response bodies. Provider usage is unknown where the official interface does not expose it.
